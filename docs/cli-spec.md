...
- Provide command payload builders for move, fire, combined fire, and end phase
...
fire_weapon        = "fire-weapon" weapon_type weapon_index target_id ;
fire_unit          = "fire-unit" unit_id target_id ;
combined_fire      = "combined-fire" unit_id { unit_id } "->" target_id ;
end_phase          = "end-phase" ;
...
fire-weapon main 0 wolf-1
fire-weapon missile 1 puss-2

fire-unit wolf-1 onion
fire-unit puss-1 main

combined-fire wolf-1 puss-1 -> main
combined-fire witch-1 wolf-2 dragon-1 -> onion
...
### Combined Fire Attacker List

Combined fire uses a variable-length attacker list terminated by `->`.

Example:

```text
combined-fire wolf-1 puss-1 witch-1 -> main
```
...
{
  "type": "COMBINED_FIRE",
  "unitIds": ["wolf-1", "puss-1", "witch-1"],
  "targetId": "main"
}
```

If `->` is missing, the CLI should reject the command locally.
...
```text
Parse error
command: combined-fire
error: expected one or more attacker unit IDs followed by '->' and a target ID
usage: combined-fire <unitId...> -> <targetId>
```
