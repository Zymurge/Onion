...
This checklist records the completed stack-roster lifecycle work in autonomous slices.
...
- [x] Centralize roster parsing and lookup.
- [x] Centralize split behavior.
- [x] Centralize merge behavior.
- [x] Centralize retire behavior.
- [x] Bridge naming allocation and retirement history through the helper.
...
- [x] Route movement-end stack consolidation through the shared lifecycle helper.
- [x] Keep stack-limit enforcement before consolidation.
- [x] Preserve surviving group identity across partial splits.
- [x] Preserve destination identity across merges.
- [x] Retire emptied groups at consolidation time.
...
- [x] Allocate finalized stack names only from authoritative naming state.
- [x] Normalize legacy group labels at load.
- [x] Retire names without reuse.
- [x] Keep unit friendly names independent from group labels.
...
- [x] Emit the canonical roster bundle in authoritative snapshots.
- [x] Emit the naming snapshot alongside it.
- [x] Rewire stack-aware projections to consume canonical roster data.
- [x] Fail fast when stack-aware views are missing canonical roster data.
...
Current status: Completed. Server snapshots emit the canonical roster and naming state; web projection helpers consume the canonical roster bundle and negative tests cover missing canonical inputs.
...
- [x] Find remaining stack-aware grouping derived from hex position alone.
- [x] Find remaining synthetic subgroup/member naming paths.
- [x] Replace them with canonical roster lookups.
- [x] Leave only explicit temporary fallbacks, if any, with tests.
...
Current status: Completed. The compatibility audit found only deliberate synthetic target projections used for combat resolution, not stack membership or member identity; stack-aware UI and engine paths use canonical roster lookups and fail fast when required data is absent.
