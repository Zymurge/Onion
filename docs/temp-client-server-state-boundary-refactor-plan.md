...
**Status:** Active; target-ID migration complete
**Date:** 2026-04-25
...
boundary is complete; the broader state-boundary work remains tracked below
until the surrounding architecture documentation is consolidated.
...
- Combat target contract completed: Onion tread targets use the canonical
	`onion-id:treads` identity across web selection, submitted commands, server
	resolution, and emitted events. Bare Onion IDs are not tread aliases.
