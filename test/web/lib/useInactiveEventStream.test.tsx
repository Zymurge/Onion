...
describe('useInactiveEventStream', () => {
	...
	it('builds rich inactive event summaries and details', async () => {
		const pollEvents = vi.fn().mockResolvedValue([
			...
			createEvent({ seq: 12, type: 'ONION_WEAPON_DESTROYED', timestamp: 't12', turnNumber: 3, weaponFriendlyName: 'Ion Cannon' }),
			createEvent({ seq: 13, type: 'ONION_WEAPON_DESTROYED', timestamp: 't13', turnNumber: 3, weaponType: 'heavy_laser' }),
		...
		expect(ramEntry?.details).toEqual(
			expect.arrayContaining([
				...
				'Target: other-1',
				'Result: survived',
				'Weapon destroyed: Ion Cannon',
				'Weapon destroyed: Heavy laser',
	...
	it('formats standalone inactive events and structured detail values', async () => {
		const pollEvents = vi.fn().mockResolvedValue([
			...
			createEvent({ seq: 4, type: 'ONION_WEAPON_DESTROYED', timestamp: 't4', turnNumber: 3, weaponType: 'heavy_laser' }),
		...
		expect(result.current.entries.map((entry) => entry.summary)).toEqual(
			expect.arrayContaining([
				...
				'unit squads lost',
				'The Onion lost the Heavy laser weapon',
				'Move by u-5',
