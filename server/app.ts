...
function resolveAdapter(db?: Partial<DbAdapter>): DbAdapter {
  ...
  return {
    ...
    archiveMatch: db?.archiveMatch?.bind(db) ?? fallback.archiveMatch.bind(fallback),
    restoreMatch: db?.restoreMatch?.bind(db) ?? fallback.restoreMatch.bind(fallback),
    deleteMatch: db?.deleteMatch?.bind(db) ?? fallback.deleteMatch.bind(fallback),
