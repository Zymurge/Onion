...
export type AxialMoveMapSnapshot = {
	width: number
	height: number
...
type Occupant = NonNullable<AxialMoveMapSnapshot['occupiedHexes']>[number]
...
function isInBounds(map: AxialMoveMapSnapshot, pos: HexPos): boolean {
	return pos.q >= 0 && pos.q < map.width && pos.r >= 0 && pos.r < map.height
}

function getTerrainLookup(map: AxialMoveMapSnapshot): Map<string, TerrainType> {
	return new Map(map.hexes.map((hex) => [hexKey(hex), terrainFromT(hex.t)]))
}

function getOccupiedLookup(map: AxialMoveMapSnapshot): Map<string, Occupant[]> {
